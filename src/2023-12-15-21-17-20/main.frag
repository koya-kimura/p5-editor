...
void main(void){
    ...
    vec2 uv0=mod(uv*floor(20.*pow(u_vol+0.1,5.)+1.),1.);
    ...
    vec2 uv1=uv;
    if(u_vol>.5&&sin(u_time+2.)>0.5){
        uv1=mod(uv*floor(20.*pow(u_vol+0.1,5.)+1.),1.);
    }
    if(u_vol>.4&&sin(u_time+1.)>.0){
        uv1=floor(uv1*300.)/300.;
    }
    if(u_vol>.5&&sin(u_time)>0.){
        uv1.y=.5;
    }
