...
function draw() {
  ...
  for(let j = 0; j < 10; j ++){
    for (let i = 0; i < n; i++) {
      const px = width / 2 + sin(frameCount / (50+j)) * 200 + cos(frameCount / 100 + i + j) * 50;
      const py = height / 2 + sin(frameCount / (60+j)) * 200 + cos(frameCount / 100 + i + j) * 50;
      const s = 50 + pow(sin(frameCount / 500 + i*14.354 + j), 100) * 500;
      fill(cp[j%cp.length]);
      circle(px - i * 50, py + i * 30, s);
    }
  }
}
