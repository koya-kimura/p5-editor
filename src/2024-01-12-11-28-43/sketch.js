...
class Bubble {
  constructor(){
    ...
    this.v = createVector(0, -random(0.1, 1));
    this.s = min(width, height)*random(0.1, 0.4);
  }

  move(){
    this.v.x += (noise(frameCount/100)-0.5)*0.001;
    this.p.add(this.v);
  ...
  display(pg){
    ...
    const d = pow(map(abs(this.p.y - height / 2), 0, height / 2, 0, 1), 2);
    pg.ellipse(this.p.x, this.p.y, this.s*(1+d));
  }
}
