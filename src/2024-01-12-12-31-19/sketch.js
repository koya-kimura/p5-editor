...
function preload(){
  img = loadImage("../../assets/image/night-shop.png")
}

function setup() {
  // const scl = img.width/img.height;
  // img.resize(windowHeight*scl, windowHeight);
...
function draw() {
  ...
  pg.push();
  pg.stroke(255);
  pg.fill(200, 200, noise(frameCount / 100) * 255);
  pg.rotateX(frameCount/100);
  pg.rotateY(frameCount / 100);
  pg.rotateZ(frameCount / 100);
  pg.box(width*0.3);
  pg.pop();
