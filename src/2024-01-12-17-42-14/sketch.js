...
function draw() {
  ...
  const n = 100;
  const xgrid = width / n;
  const ygrid = height / colorPalletes.length;

  for (let j in colorPalletes) {
    let cp = colorPalletes[j].colors;
    for (let i in cp) {
      noFill();
      stroke(cp[i]);
      beginShape();
      for (let x = -100; x < width+100; x += xgrid) {
        curveVertex(x, ygrid * j + ygrid / 2 + sin(x / 100 + frameCount / 100 + sin(i / 3 + j / 3 + frameCount / 100)) * ygrid / 6 + cos(x / 50 + frameCount / 200 + sin(i / 5 + j / 7 + frameCount / 200)) * ygrid / 6)
      }
      endShape();
    }
  }
