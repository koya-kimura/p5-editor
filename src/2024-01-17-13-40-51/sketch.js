function setup() {
  ...
  drawSeparateNgonPattern(0, 0, width, n, pre, cp)
}

function drawSeparateNgonPattern(_x, _y, _s, _n, _pre, _cp) {
  for (let x = 0; x < _s; x += _s / 2) {
    for (let y = 0; y < _s; y += _s / 2) {
      if (_s / 2 > 70 && random(1000) < 700) {
        drawSeparateNgonPattern(_x + x, _y + y, _s / 2, _n, _pre, _cp);
      } else {
        // circle(_x + x + _s / 4, _y +y + _s / 4, _s / 2);
        drawPreNgon(_n, _x + x +  _s / 4, _y + y +  _s / 4, _s / 4, _pre, _cp);
      }
    }
  }
}
