function setup() {
  ...
  const r = width * 0.7;
  let start = 0;
  let end = random(PI / 10, PI / 2);
  for (let th = end; th < TAU; th += random(PI / 10, PI / 2)) {
    end = th;
    let n = floor(random(3, 10));
    let isFill = random(1000) < 800;
    for (let i = n; i > 0; i--) {
      if (isFill) {
        const c = random(cp);
        stroke(c);
      ...
      arc(width / 2, height / 2, r * i / n, r * i / n, start, end);
    }
    start = end;
  }
  ...
  for (let i = n; i > 0; i--) {
    ...
    arc(width / 2, height / 2, r * i / n, r * i / n, start, TAU);
  }
}
