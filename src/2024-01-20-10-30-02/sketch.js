...
function setup() {
  ...
  for(let x = grid/2; x < width; x += grid){
    for(let y = grid/2; y < height; y += grid){
      ...
      if(g > 10){
        bubble.push(new Bubble(x, y, grid * 0.8, random(cp)));
      }
    }
  }
}
...
function draw() {
  ...
  camera(0, 1000 * cos(frameCount / 100), 1000, 0, 0, 0, 0, 1, 0);
...
class Bubble {
  constructor(x, y, s, c){
    this.p = createVector(x-width/2, y-height/2, 0);
    this.v = createVector(0, 0, 0);
    this.s = s;
    this.c = c;
    this.r = s * random(0.05, 0.1);
  }

  move(){
    this.p.add(this.v);
    this.p.x = this.p.x + this.r * cos(frameCount / 100);
    this.p.z = this.r * sin(frameCount / 100);

    if(this.p.y < -height/2 - 100){
      this.p.y = height/2 + 100;
    }
  }
