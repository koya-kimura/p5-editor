...
function setup() {
  ...
  center = createVector(width * 0.7, height * 0.5);
}
...
function draw() {
  ...
  for (let i = 0; i < 30; i++) {
    fill(cp[i%cp.length])
    circle(center.x + cos(frameCount / 100 + i) * frameCount, center.y + sin(frameCount / 100 + i) * frameCount, frameCount / 10 + noise(frameCount/100)*10);
  }
}
