...
function setup() {
  ...
  const x = width / 2;
  const period = floor(random(10, 20));
  for (let i = 0; i < 5000; i++) {
    const c = random(cp);
    const vy = random(5, 10);
    seed.push(new Seed(x, vy, c, period));
  }
...
class Seed {
  constructor(x, vy, c, period) {
    this.p = createVector(x+min(width, height)*random(-0.1, 0.1), height*0.98);
    this.v = createVector(0, vy);
    this.c = c;
    this.s = pow(random(), 3)*20;
    this.g = random(0.1, 3);
    this.period = period;
  }
  ...
  display() {
    const a = pow(map(abs(this.p.x - width / 2), 0, width/2, 0, 1), 2)*30 + 1;
    const s = pow(map(this.p.y, 0, height, 1, 0), 2)*20+5;
    noStroke();
