...
function draw() {
  ...
  for(let x = grid/2; x < width; x += grid){
    for (let y = grid / 2; y < height; y += grid) {
      const r = grid * 0.5 * scl;
      const angle = frameCount/100;
      const p0 = createVector(x + r * cos(angle), y + r * sin(angle));
      const p1 = createVector(x + r * cos(angle+PI), y + r * sin(angle+PI));

      strokeWeight(floor(pow(noise(x/500, y/500, frameCount/100), 2)*5+1))
      stroke(255);
