...
function draw() {
  ...
  const grid = 80;
  let index = 0;
  for(let x = grid*1.5; x < width-grid; x += grid){
    for (let y = grid*1.5; y < height-grid; y += grid) {
      const s = grid * 0.7;
      const offset = s*0.2;
      noStroke();
      fill(0);
      circle(x + cos(frameCount / 100 + x/500) * offset, y + sin(frameCount / 100 + y/500) * offset, s * 0.95);
      // fill(cp[index%cp.length]);
      fill(250);
      circle(x, y, s);
      index ++;
    }
