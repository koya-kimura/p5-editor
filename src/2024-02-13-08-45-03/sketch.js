...
function setup() {
  ...
  for(let r = 0.2; r < 1; r += 0.2){
    const c = random(cp);
    for (let th = 0; th < TAU; th += TAU / (50*r)) {
      const option = {
        r:min(width, height)*r*0.5,
        th:th,
        c:c,
        v:r*0.01
      }
      particle.push(option);
    }
  }
}
...
function draw() {
  ...
  for (let p1 of particle) {
    for (let p2 of particle) {
      push();
      translate(width / 2, height / 2);
      const x1 = p1.r * cos(p1.th);
      const y1 = p1.r * sin(p1.th);
      const x2 = p2.r * cos(p2.th);
      const y2 = p2.r * sin(p2.th);
  ...
  for(let p of particle){
    ...
    const x = p.r*cos(p.th);
    const y = p.r*sin(p.th);
    circle(x, y, 10);
    pop();
  }
}
