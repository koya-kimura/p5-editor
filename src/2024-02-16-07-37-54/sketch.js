...
function setup() {
  ...
  for (let x = grid / 2; x < width; x += grid) {
    for (let y = grid / 2; y < height; y += grid) {
      const c = img.get(x, y);
      arr.push({x:x, y:y, r:grid*1.5, c:c});
    }
  }
  arr = shuffle(arr);
...
function draw() {
  for(let t = 0; t < 8000; t ++){
    stroke(red(arr[index].c) + random(-30, 30), green(arr[index].c) + random(-30, 30), blue(arr[index].c) + random(-30, 30));
    line(arr[index].x, arr[index].y, arr[index].x + cos(angle) * arr[index].r, arr[index].y + sin(angle) * arr[index].r);
