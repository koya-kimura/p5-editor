...
function preload(){
  ...
  img[0] = loadImage("../../assets/image/night-shibuya.png");
  img[1] = loadImage("../../assets/image/night-shop.png");
  img[2] = loadImage("../../assets/image/night-meat.png");
  img[3] = loadImage("../../assets/image/night-crossing.png");
  img[4] = loadImage("../../assets/image/night-car.png");
  img[5] = loadImage("../../assets/image/night-window.png");
...
function setup() {
  ...
  pg.imageMode(CENTER);
  for(let i in img){
    img[i].resize(img[i].width * 3.5, img[i].height * 3.5);
  }
...
function draw() {
  ...
  const t = Easing.easeInOutExpo((frameCount % period) / period);
  ...
  if(t==0){
    ...
    // period = floor((1-pow(spm[10], 2))*20)+30;
  }
  ...
  const x = map(t, 0, 1, x1, x2);
  const y = map(t, 0, 1, y1, y2);

  pg.image(img[index%img.length], x, y);

  shader(theShader);

  const dir = createVector(x2-x1, y2-y1).normalize();
  theShader.setUniform("u_time", t);
  theShader.setUniform("u_tex", pg);
  theShader.setUniform("u_dir", [dir.x, dir.y]);
