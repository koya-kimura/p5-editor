...
function preload(){
  img[0] = loadImage("../../assets/image/night-car.png");
  // img[1] = loadImage("../../assets/image/night-shibuya.png");
  // img[2] = loadImage("../../assets/image/night-crossing.png");
}
...
function setup() {
  ...
  for (let i in img) {
    img[i].resize(width * 2, 0);
  }

  for(let x = s/2; x < width; x += s){
    for (let y = s / 2; y < height; y += s) {
      const c = img[0].get(x, y);
      particle.push(new Particle(x, y, c, c, s));
    }
  }
...
class Particle {
  constructor(x, y, c1, c2, s){
    this.initPos = createVector(x, y);
    this.pos = createVector(x, y);
    this.col1 = c1;
    this.col2 = c2;
    this.col = this.col1;
    this.size = s * random(0.8, 1.2);
  ...
  display(){
    noStroke();
    fill(this.col);
    circle(this.pos.x, pmod(this.pos.y, height), this.size);
  }
}
