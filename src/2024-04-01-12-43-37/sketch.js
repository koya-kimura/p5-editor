...
function setup() {
  ...
  for(let i = 0; i < 100; i ++){
    pg.fill(random(cp));
    pg.noStroke();
    pg.circle(random(pg.width), random(pg.height), pg.width * random(0.05, 0.2));
  }
}
...
function draw() {
  ...
  const radius = 500 * (pow(cos(frameCount / 100), 2) + 1);
  camera(radius * cos(frameCount / 50), radius * cos(frameCount / 50), radius * sin(frameCount / 50), 0, 0, 0, 0, 1, 0);
