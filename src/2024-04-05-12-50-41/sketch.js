...
function setup() {
  ...
  for(let i = 0; i < 200; i ++){
    pg_arr[i] = createGraphics(w, w);

    // pg_arr[i].background(random(cp));

    pg_arr[i].noStroke();
    pg_arr[i].fill(255);
    pg_arr[i].circle(w *0.5, w *0.5, w * 0.11);

    pg_arr[i].push();
    pg_arr[i].beginClip();
    pg_arr[i].circle(w/2, w/2, w*0.1);
    pg_arr[i].endClip();

    for(let j = 0; j < 100; j ++){
      pg_arr[i].strokeCap(PROJECT);
      pg_arr[i].stroke(random(cp));
      pg_arr[i].strokeWeight(w*random(0.01, 0.05));
      pg_arr[i].line(random(w), random(w), random(w), random(w));
    }
    pg_arr[i].pop();
  }

  for(let pg of pg_arr){
    imageMode(CENTER);
    push();
